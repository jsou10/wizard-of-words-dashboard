#!/usr/bin/env python3
"""
Wizard of Words Dashboard : Vercel serverless edition (moved off Render 2026-09-20).

Same data brain as app.py with the serverless changes: no background threads,
/api/data computes synchronously (maxDuration 300 in vercel.json) and caches in
memory + /tmp while the instance is warm (DATA_CACHE_SECONDS, default 30 min;
?force=1 bypasses). Budgets live in Supabase table dashboard_budgets under
keys "wow:{brand}-{num}" because serverless has no disk.

Original: — live-API edition.
...
DATA_CACHE_TTL = int(os.environ.get("DATA_CACHE_SECONDS", "1800"))  # /api/data expensive — cache 5m
...
class _VercelPathRestore:
    """Vercel's rewrite hands the function the DESTINATION path (/api/index),
    not the URL the browser asked for. vercel.json appends the real path as
    ?__path=..., and this middleware puts it back into PATH_INFO before Flask
    routes the request (verified 2026-09-20; older builds preserved the path)."""
    ...
    def __call__(self, environ, start_response):
        ...
        if "__path=" in qs:
            ...
            real = [v for k, v in pairs if k == "__path"]
            if real:
                environ["PATH_INFO"] = real[-1]
                environ["QUERY_STRING"] = urlencode([(k, v) for k, v in pairs if k != "__path"])
        return self.wsgi(environ, start_response)

app.wsgi_app = _VercelPathRestore(app.wsgi_app)
...
# ─── Cache (in-memory + /tmp; both survive only while the instance is warm) ──
_cache: dict = {}
CACHE_DIR = "/tmp/wow-cache"
...
def cache_peek_raw(key):
    """Raw entry (value + ts), ignoring TTL. Lets /api/pacing read the last
    computed data even after the data cache expired."""
...
def cache_set(key, value, ttl=CACHE_TTL):
    entry = {"value": value, "ts": time.time(), "ttl": ttl}
    _cache[key] = entry
    try:
        with open(_cache_path(key), "w") as f:
            json.dump(entry, f)
    except Exception as e:
        log("warn", "cache_tmp_write_failed", error=str(e))

# ─── Budget storage (Supabase; Vercel serverless has no disk) ────────────────
SUPABASE_URL = (os.environ.get("SUPABASE_URL", "") or "").rstrip("/")
SUPABASE_SERVICE_KEY = os.environ.get("SUPABASE_SERVICE_KEY", "")
BUDGET_KEY_PREFIX = "wow:"   # rows look like "wow:GX-17", "wow:WoW-33"
...
def load_event_budgets():
    """Returns {event_key: total_budget_dollars}, e.g. {"GX-17": 2000.0}."""
    if not (SUPABASE_URL and SUPABASE_SERVICE_KEY):
        return {}
    try:
        res = requests.get(
            f"{SUPABASE_URL}/rest/v1/dashboard_budgets",
            params={"select": "key,total_budget", "key": f"like.{BUDGET_KEY_PREFIX}*"},
            headers=_sb_headers(), timeout=(5, 10),
        )
        res.raise_for_status()
        return {row["key"][len(BUDGET_KEY_PREFIX):]: float(row["total_budget"]) for row in res.json()}
    except Exception as e:
        log("warn", "load_budgets_failed", error=str(e))
        return {}

def save_event_budget(event_key, total):
    """Upsert one budget row; total <= 0 deletes it."""
    key = f"{BUDGET_KEY_PREFIX}{event_key}"
    if total <= 0:
        res = requests.delete(
            f"{SUPABASE_URL}/rest/v1/dashboard_budgets",
            params={"key": f"eq.{key}"}, headers=_sb_headers(), timeout=(5, 10),
        )
    else:
        res = requests.post(
            f"{SUPABASE_URL}/rest/v1/dashboard_budgets",
            ...
            json={"key": key, "total_budget": total,
...
def api_data():
    ...
    try:
        ...
        stale = cache_peek_raw(cache_key)
        if stale:
...
def api_set_budget(key):
    ...
    try:
        save_event_budget(key, total)
    except Exception as e:
        log("error", "save_budget_failed", event_key=key, error=str(e))
        return jsonify({"error": f"save failed: {str(e)[:200]}"}), 502
...
# (Startup validation thread removed: serverless. /api/health covers it.)
